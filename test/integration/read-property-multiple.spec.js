...
describe("bacnet - readPropertyMultiple integration", () => {
  ...
  it("should successfully decode a structured view", (next) => {
    ...
    client.readPropertyMultiple("127.0.0.2", requestArray, (err, response) => {
      ...
      expect(response.values[0].objectId).to.deep.equal({
        type: 29,
        instance: 31,
      });
      expect(object[28]).to.deep.equal([
        {
      ...
      expect(object[75]).to.deep.equal([
        { value: { type: 29, instance: 31 }, type: 12 },
      ]);
      expect(object[77]).to.deep.equal([
        {
      ...
      expect(object[168]).to.deep.equal([
  ...
  it("should successfully decode a value object", (next) => {
    ...
    client.readPropertyMultiple("127.0.0.2", requestArray, (err, response) => {
      ...
      expect(object[77]).to.deep.equal([
        { value: "Zählerweg'Floor 4'RSegTV'SenDev'Sen", type: 7, encoding: 0 },
      ]);
      ...
      expect(object[111]).to.deep.equal([
