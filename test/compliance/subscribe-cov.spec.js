    ...
    bacnetClient.subscribeCov(
      ...
      (err) => {
        expect(err.message).to.equal(
          "BacnetError Class: OBJECT (1) Code: OPTIONAL_FUNCTIONALITY_NOT_SUPPORTED (45)"
        );
        expect(err.bacnetErrorClass).to.equal(
          utils.bacnetClient.enum.ErrorClass.OBJECT
        ); // 1
        expect(err.bacnetErrorCode).to.equal(
          utils.bacnetClient.enum.ErrorCode.OPTIONAL_FUNCTIONALITY_NOT_SUPPORTED
        ); // 45
        next();
      }
    );
