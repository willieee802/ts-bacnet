  ...
  it("read all properties from invalid device, expect errors in response", (next) => {
    ...
    bacnetClient.readPropertyMultiple(
      ...
      (err, value) => {
        ...
        expect(value.values[0].objectId).to.deep.equal({
          type: 8,
          instance: utils.deviceUnderTest + 1,
        });
        expect(value.values[0].values).to.be.an("array");
        expect(value.values[0].values[0]).to.deep.equal({
          id: 75,
          index: utils.index,
          value: [{ type: 105, value: { errorClass: 1, errorCode: 31 } }],
        });
        next();
      }
    );
    ...
    bacnetClient.readPropertyMultiple(
      ...
      (err, value) => {
        ...
        expect(value.values[0].values[0]).to.deep.equal({
          ...
          index: utils.index,
          value: [
            { value: { type: 8, instance: utils.deviceUnderTest }, type: 12 },
          ],
        });
        next();
      }
    ...
    bacnetClient.readPropertyMultiple(
      ...
      (err, value) => {
        ...
        expect(value.values[0].values[0]).to.deep.equal({
          id: 75,
          index: utils.index,
          value: [{ value: { type: 1, instance: 2 }, type: 12 }],
        });
        expect(value.values[0].values[1]).to.deep.equal({
          id: 77,
          index: utils.index,
          value: [{ value: "ANALOG OUTPUT 2", type: 7, encoding: 0 }],
        });
        next();
      }
    );
